function abrirAula(card){
    ...
    if(idVideo == 1){  
        ...
        <div class="videoAula">
                <input type="checkbox" id="porcentagem_input" onclick="atualizarProgresso()"><p>Após assistir o video, marque a caixa para sabermos quantos porcentos você está avançando no curso</p>
        </div>`;       
    }
    else if(idVideo == 2){
        boxes.innerHTML = `
...
function atualizarProgresso() {
        ...
        if(aulaVista){
                ...
                for(var progressoAtual = 0; progressoAtual < vetorProgresso.length; progressoAtual++){
                        ...
                        porcentagem.innerHTML = `${mostrarProgresso()}%`;
        ...
        fetch("/progresso/barraProgresso", {
            ...
            body: JSON.stringify({
                idAlunoServer: sessionStorage.ID_USUARIO,
                porcentagemProgressoServer: porcentagemProgresso,
            })
        })
}
